...
public class UpdateJoinEvent implements Listener
{
    ...
    public void onJoin(final PlayerJoinEvent e) {
    	...
    	if (p.hasPermission("sbf.update")) {
    		if (SBF.getPlugin().getConfig().getBoolean("Update.Enabled") == true){
    		this.checker = new UpdateChecker(SBF.plugin);
                        if (this.checker.isConnected()) {
                            if (this.checker.hasUpdate()) {
                                ...
                                p.sendMessage(ChatColor.RED + "Your version: " + SBF.plugin.getDescription().getVersion());
                                p.sendMessage(ChatColor.GRAY + "=========================");
                            }
                        }               
       }
